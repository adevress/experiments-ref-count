...
public:
    ...
    Durc(const Durc & other) : underlying_(other.underlying_){
        inc_if_non_null(underlying_);
    }
    
    Durc(Durc && other) : underlying_(other.underlying_){
        other.underlying_ = nullptr;
    }
    
    ~Durc(){
        underlying_ = dec_if_non_null(underlying_);
    }
...
private:
    ...
    static Impl* create_from_ptr(T* d){
        Impl* impl = new Impl{
            // accept one move or one copy here if constructed from 
